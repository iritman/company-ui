...
import SearchModal from "./service-requests-search-modal";
import DetailsModal from "./service-request-details-modal";
...
const ServiceRequestsPage = ({ pageName }) => {
  const {
    progress,
    setProgress,
    searched,
  ...
  const handleSaveItem = async (request_item) => {
    ...
    const service_request_index = records.findIndex(
      (service_request) => service_request.RequestID === request_item.RequestID
    );

    records[service_request_index] = rec;
  ...
  const handleDeleteItem = async (item_id) => {
    ...
    if (selectedObject) {
      ...
      const service_request_index = records.findIndex(
        (service_request) => service_request.RequestID === rec.RequestID
      );
  ...
  const handleApprove = async () => {
    ...
    try {
      const data = await service.approveServiceRequest(
  ...
  const handleUndoApprove = async () => {
    ...
    try {
      const data = await service.undoApproveServiceRequest(
        selectedObject.RequestID
      );

      // Update selected object
      selectedObject.StatusID = 1; // In progress
      selectedObject.StatusTitle = Words.service_request_status_1;
      setSelectedObject({ ...selectedObject });
  ...
  const handleReject = async () => {
    ...
    try {
      const data = await service.rejectServiceRequest(selectedObject.RequestID);

      // Update selected object
      selectedObject.StatusID = 3; // Reject
      selectedObject.StatusTitle = Words.service_request_status_3;
      setSelectedObject({ ...selectedObject });
    ...
    <>
      <Spin spinning={progress}>
        <Row gutter={[10, 15]}>
          <SimpleDataPageHeader
            title={Words.service_requests}
            sheets={getSheets(records)}
            fileName="ServiceRequests"
      ...
      {showModal && (
        <ServiceRequestModal
          ...
          onSaveServiceRequestItem={handleSaveItem}
          onDeleteServiceRequestItem={handleDeleteItem}
