...
import {
  ...
  personalTransferActionFilesUrl,
} from "../../../../../config.json";
...
const PersonalTransferDetails = ({ transfer, bannedSteps }) => {
      ...
      content: (
        ...
        >
          ...
          <Descriptions.Item label={Words.reg_member}>
            ...
            >{`${TransferFirstName} ${TransferLastName}`}</Text>
          </Descriptions.Item>
          ...
          <Descriptions.Item label={Words.from_department}>
            <Text style={{ color: Colors.blue[6] }}>{FromDepartmentTitle}</Text>
          </Descriptions.Item>
          <Descriptions.Item label={Words.from_role}>
            <Text style={{ color: Colors.blue[6] }}>{FromRoleTitle}</Text>
          </Descriptions.Item>
          <Descriptions.Item label={Words.to_department}>
            <Text style={{ color: Colors.green[6] }}>{ToDepartmentTitle}</Text>
          </Descriptions.Item>
          <Descriptions.Item label={Words.to_role}>
            <Text style={{ color: Colors.blue[6] }}>{ToRoleTitle}</Text>
          </Descriptions.Item>
          ...
          <Descriptions.Item label={Words.receiving_properties} span={2}>
            <Text
              style={{
                color: Colors.purple[7],
                whiteSpace: "pre-line",
            ...
            <Descriptions.Item label={Words.attached_files} span={2}>
              <Col xs={24}>
                {Files.map((f) => (
                  <a
                    key={f.FileID}
                    href={`${personalTransferFilesUrl}/${f.FileName}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <Tag
                ...
                <Descriptions.Item label={Words.attached_files} span={2}>
                  <Col xs={24}>
                    {action.Files.map((f) => (
                      <a
                        key={f.FileID}
                        href={`${personalTransferActionFilesUrl}/${f.FileName}`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        <Tag
    ...
    <Row gutter={[10, 10]}>
      <Col xs={24}>
        <Alert
          message={
            <Space>
              <MemberProfileImage fileName={TransferPicFileName} />
              <Text>{`${TransferFirstName} ${TransferLastName}`}</Text>
            </Space>
          }
          type="info"
        />
...
export default PersonalTransferDetails;
