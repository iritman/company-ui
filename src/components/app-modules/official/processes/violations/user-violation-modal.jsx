...
import {
  violationFilesUrl,
  violationFileConfig as fileConfig,
} from "./../../../../../config.json";
import { onUpload } from "../../../../../tools/upload-tools";

const schema = {
  ViolationID: Joi.number().required(),
  ViolationMemberID: Joi.number().min(1).required(),
...
const UserViolationModal = ({ isOpen, selectedObject, onOk, onCancel }) => {
      ...
      selectedObject.Files.forEach((f) => {
        files = [
          ...files,
          {
            ...
            url: `${violationFilesUrl}/${f.FileName}`,
            FileID: f.FileID,
            FileName: f.FileName,
            FileSize: f.FileSize,
          },
  ...
  const handleSubmit = async () => {
    ...
    if (data.error) {
      ...
      if (selectedObject) {
        const saved_files = await service.getViolationFiles(
          selectedObject.ViolationID
        );

        saved_files.forEach((f) => {
          f.uid = f.FileID;
          f.name = Words.attached_file;
          f.url = `${violationFilesUrl}/${f.FileName}`;
        });
...
export default UserViolationModal;
