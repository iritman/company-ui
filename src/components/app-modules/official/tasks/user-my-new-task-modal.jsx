...
const UserMyNewTaskModal = ({
  ...
  const handleDoneTask = async () => {
    await onDone(selectedObject.TaskID);
  ...
  const getFooterButtons = () => {
    ...
    if (selectedObject?.DoneDate.length === 0) {
      buttons = [
        ...buttons,
        <Popconfirm
          title={Words.questions.sure_to_make_task_done}
          onConfirm={handleDoneTask}
          okText={Words.yes}
          cancelText={Words.no}
          icon={<QuestionIcon style={{ color: "red" }} />}
        >
          <Button key="submit-done-button" type="primary" loading={progress}>
...
export default UserMyNewTaskModal;
